...
IRAM_ATTR void FloatBiquadFilter::configure(
	BiquadFilterType type,
	float            cutoff,
	float            resonance
) {
...
IRAM_ATTR void FloatBiquadFilter::configurePeaking(
	float cutoff,
	float resonance,
	float gain
) {
...
IRAM_ATTR void FloatBiquadFilter::reset(void) {
	sa1_ = 0;
	sa2_ = 0;
	sb1_ = 0;
	sb2_ = 0;
...
IRAM_ATTR float FloatBiquadFilter::update(float value) {
	float filtered = b0_ * value;
	filtered      += b1_ * sb1_;
	filtered      += b2_ * sb2_;
	filtered      -= a1_ * sa1_;
	filtered      -= a2_ * sa2_;
...
IRAM_ATTR void WaveformEncoder::reset(void) {
	accumulator_ = 0;
	currentPeak_ = 0;
	lastNibble_  = -1;
}

IRAM_ATTR size_t WaveformEncoder::encode(
	uint8_t      *output,
	const Sample *input,
	int          sampleRate,
	size_t       numSamples,
