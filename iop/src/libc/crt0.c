...
extern char _romDataStart[];
extern char _dataStart[], _dataEnd[];
...
static uintptr_t _heapLimit = 0x20000800; // TODO: add a way to change this
...
int _startInner(int argc, const char **argv) {
	// Relocate all preinitialized variables from flash to RAM and set all
	// uninitialized variables to zero by clearing the BSS section.
	__builtin_memcpy(_dataStart, _romDataStart, _dataEnd - _dataStart);
